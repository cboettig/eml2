...
#'
#' @param data (data.frame) A data.frame of data that needs an attribute table
...
#' Get EML numberType
#'
#' @description Returns the EML numberType (either 'real', 'integer', 'whole', or 'natural') of input values
#'
#' @param values  (numeric/character) a vector of values, if vector is non-numeric will return NA
#' @return (character) the numberType of \code{values} (either 'real', 'integer', 'whole', or 'natural').
#' @examples
#' \dontrun{
...
get_numberType <- function(values) {
  # Unlist data. Uses do.call to preserve date format
  if (is.list(values) == T) {
...
#' Hot table to r
#'
#' @description Takes a hot handsontable table and converts to r data.frame
#'
