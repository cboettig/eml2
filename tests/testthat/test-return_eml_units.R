...
testthat::test_that("return_eml_units accepts multiple unit input formats", {
  ...
  form_1 <- return_eml_units('kilometersPerSquareSecond')
...
testthat::test_that("return_eml_units recognizes all EML units", {

  input_units <- EML::get_unitList()$units
  out <- return_eml_units(unlist(input_units$id))

  testthat::expect_equal(out, input_units[, colnames(out)])
})
